...
button :: (ReflexSDL2 t m, MonadDynamicWriter t [Layer m] m, MonadReader Renderer m) => m (Event t ButtonState)
button = do
...
guest = do
  ...
  commitLayer $ ffor dMoves $ \moves ->
    forM_ (reverse moves) $ \dat -> do
      renderAABB r (V4 255 255 0 128) (V2 100 100)
  ...
  dBtns         <- foldDyn (\x xs -> take 10 $ x : xs) [] evMouseButton
...
app = do
  ...
  performEvent_ $ ffor evMouseMove $ \layers -> do
    rendererDrawColor r $= V4 0 0 0 255
    clear r
    -- sequence_ layers
    present r
