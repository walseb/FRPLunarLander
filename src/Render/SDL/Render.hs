...
render :: (Number a) => S.Renderer -> Resources -> (GameState a, Bool) -> IO Bool
render renderer res (GameState (CameraState zoomLevel) (PhysicalState (Player _ pCollObj' score' fuel') (Scene terrain' landingSpots')), exit) =
  do
    ...
    renderText' ("score: " ++ show score') (S.Rectangle (S.P (V2 100 100)) (V2 150 100)) (V4 255 255 255 255)
    renderText' ("Fuel: " ++ show (floor (if fuel' < 0 then 0 else fuel'))) (S.Rectangle (S.P (V2 100 200)) (V2 150 100)) (V4 255 255 255 255)
    S.present renderer
    pure exit
  where
